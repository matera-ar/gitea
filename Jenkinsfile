...
def runMake(task) {
    runShellBasedCommand("'docker login --username \\\$DOCKER_USERNAME --password \\\$DOCKER_PASSWORD harbor-dev.matera.com && TAGS=\\\"bindata sqlite sqlite_unlock_notify\\\" make ${task}'")
}
